...
#IBS matrix used fitered sites and run on phase 2 tank E2 females

#set working directory
setwd("/stor/work/Hofmann/projects/aburtoni_paternity_testing/Paternity_Testing/2bRAD_data/Data/Matrix/Phase2/TankE2")
...
samples=read.delim('bamsphase2e2females',header=F)
...
#load in ibs matrix
m.ibs <- as.matrix(read.table("IBS.TankE2.females.ibsMat"))
colnames(m.ibs)=sample.data$Sample.name
rownames(m.ibs)=sample.data$Sample.name

#load in covariance matrix
m.cov <- as.matrix(read.table("IBS.TankE2.females.covMat"))
...
#subset IBS matrix by phase 2 Tank E2
...
m.ibs.E2.males=as.data.frame(m.ibs)
m.ibs.E2.males=m.ibs.E2.males[rownames(m.ibs.E2.males) %in% phase.2.E2.names.males,colnames(m.ibs.E2.males) %in% phase.2.E2.names.males]
m.ibs.E2.males=data.matrix(m.ibs.E2.males)

png("pvclut.phase.2.tank.E2.males.png", width = 1800, height = 700)
plot(pvclust(m.ibs.E2.males, method.hclust = "average",method.dist = 'euclidian'), main='Phase 2 Tank E2 Males')
dev.off()
##########
...
g=graph_from_adjacency_matrix(m.ibs.E2.no.males,mode='undirected',weighted = TRUE)
layout <- layout.mds(g, dist = as.matrix(m.ibs.E2.no.males))
...
png("mds.network.phase.2.tank.E2.no.males.png", width = 1800, height = 700)
plot(g, layout = layout, vertex.size = 3, edge.label=round(E(g)$weight, 3), main='Tank E2 Females')
dev.off()

#just edges under 0.2
m.ibs.E2.no.males.2=ifelse(m.ibs.E2.no.males>=0.2,0,m.ibs.E2.no.males)
h=graph_from_adjacency_matrix(m.ibs.E2.no.males.2,mode='undirected',weighted = TRUE)
...
png("cut.mds.network.phase.2.tank.E2.no.males.png", width = 1800, height = 700)
plot(h, layout = layout, vertex.size = 3, edge.label=round(E(h)$weight, 3), main= 'Tank E2 Females Cut')
dev.off()

#subset IBS matrix by phase 1 females
m.ibs.E2.females=as.data.frame(m.ibs)
m.ibs.E2.females=m.ibs.E2.females[rownames(m.ibs.E2.females) %in% phase.2.E2.names.females,colnames(m.ibs.E2.females) %in% phase.2.E2.names.females]
m.ibs.E2.females=data.matrix(m.ibs.E2.females)

png("pvclut.phase.2.tank.E2.females.png", width = 1800, height = 700)
plot(pvclust(m.ibs.E2.females, method.hclust = "average",method.dist = 'euclidian'), main='Phase 2 Tank E2 females')
dev.off()


#network undirected matrix with edge's labeled
g=graph_from_adjacency_matrix(m.ibs.E2.females,mode='undirected',weighted = TRUE)
layout <- layout.mds(g, dist = as.matrix(m.ibs.E2.females))

png("mds.network.phase.2.tank.E2.females.png", width = 1800, height = 700)
plot(g, layout = layout, vertex.size = 3, edge.label=round(E(g)$weight, 3), main='Tank E2 Females')
dev.off()

#just edges under 0.2
m.ibs.E2.females.2=ifelse(m.ibs.E2.females>=0.2,0,m.ibs.E2.females)
h=graph_from_adjacency_matrix(m.ibs.E2.females.2,mode='undirected',weighted = TRUE)

png("cut.mds.network.phase.2.tank.E2.females.png", width = 1800, height = 700)
plot(h, layout = layout, vertex.size = 3, edge.label=round(E(h)$weight, 3), main= 'Tank E2 Females Cut')
dev.off()
