...
#IBS matrix used fitered sites and run on phase 2 tank E2 males
######
...
samples=read.delim('bamsphase2e2males',header=F)
...
g=graph_from_adjacency_matrix(m.ibs.E2.no.females,mode='undirected',weighted = TRUE)
layout <- layout.mds(g, dist = as.matrix(m.ibs.E2.no.females))
...
png("mds.network.phase.2.tank.E2.no.females.png", width = 1800, height = 700)
plot(g, layout = layout, vertex.size = 3, edge.label=round(E(g)$weight, 3), main='Tank E2 Males')
dev.off()

#just edges under 0.2
m.ibs.E2.no.females.2=ifelse(m.ibs.E2.no.females>=0.2,0,m.ibs.E2.no.females)
h=graph_from_adjacency_matrix(m.ibs.E2.no.females.2,mode='undirected',weighted = TRUE)
...
png("cut.mds.network.phase.2.tank.E2.no.females.png", width = 1800, height = 700)
plot(h, layout = layout, vertex.size = 3, edge.label=round(E(h)$weight, 3), main= 'Tank E2 Males Cut')
dev.off()
...
#network undirected matrix with edge's labeled
g=graph_from_adjacency_matrix(m.ibs.E2.males,mode='undirected',weighted = TRUE)
layout <- layout.mds(g, dist = as.matrix(m.ibs.E2.males))

png("mds.network.phase.2.tank.E2.males.png", width = 1800, height = 700)
plot(g, layout = layout, vertex.size = 3, edge.label=round(E(g)$weight, 3), main='Tank E2 Males')
dev.off()

#just edges under 0.2
m.ibs.E2.males.2=ifelse(m.ibs.E2.males>=0.2,0,m.ibs.E2.males)
h=graph_from_adjacency_matrix(m.ibs.E2.males.2,mode='undirected',weighted = TRUE)

png("cut.mds.network.phase.2.tank.E2.males.png", width = 1800, height = 700)
plot(h, layout = layout, vertex.size = 3, edge.label=round(E(h)$weight, 3), main= 'Tank E2 Males Cut')
dev.off()
##########
