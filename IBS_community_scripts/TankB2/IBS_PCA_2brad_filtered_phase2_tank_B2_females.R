...
#IBS matrix used fitered sites and run on phase 2 tank B2 females
...
samples=read.delim('bamsphase2b2females',header=F)
...
#load in ibs matrix
m.ibs <- as.matrix(read.table("IBS.TankB2.females.ibsMat"))
colnames(m.ibs)=sample.data$Sample.name
rownames(m.ibs)=sample.data$Sample.name

#load in covariance matrix
m.cov <- as.matrix(read.table("IBS.TankB2.females.covMat"))
...
g=graph_from_adjacency_matrix(m.ibs.B2.no.males,mode='undirected',weighted = TRUE)
layout <- layout.mds(g, dist = as.matrix(m.ibs.B2.no.males))
...
png("mds.network.phase.2.tank.B2.no.males.png", width = 1800, height = 700)
plot(g, layout = layout, vertex.size = 3, edge.label=round(E(g)$weight, 3), main='Tank B2 Females')
dev.off()

#just edges under 0.2
m.ibs.B2.no.males.2=ifelse(m.ibs.B2.no.males>=0.2,0,m.ibs.B2.no.males)
h=graph_from_adjacency_matrix(m.ibs.B2.no.males.2,mode='undirected',weighted = TRUE)
...
png("cut.mds.network.phase.2.tank.B2.no.males.png", width = 1800, height = 700)
plot(h, layout = layout, vertex.size = 3, edge.label=round(E(h)$weight, 3), main= 'Tank B2 Females Cut')
dev.off()
...
#network undirected matrix with edge's labeled
g=graph_from_adjacency_matrix(m.ibs.B2.females,mode='undirected',weighted = TRUE)
layout <- layout.mds(g, dist = as.matrix(m.ibs.B2.females))

png("mds.network.phase.2.tank.B2.females.png", width = 1800, height = 700)
plot(g, layout = layout, vertex.size = 3, edge.label=round(E(g)$weight, 3), main='Tank B2 Females')
dev.off()

#just edges under 0.2
m.ibs.B2.females.2=ifelse(m.ibs.B2.females>=0.2,0,m.ibs.B2.females)
h=graph_from_adjacency_matrix(m.ibs.B2.females.2,mode='undirected',weighted = TRUE)

png("cut.mds.network.phase.2.tank.B2.females.png", width = 1800, height = 700)
plot(h, layout = layout, vertex.size = 3, edge.label=round(E(h)$weight, 3), main= 'Tank B2 Females Cut')
dev.off()