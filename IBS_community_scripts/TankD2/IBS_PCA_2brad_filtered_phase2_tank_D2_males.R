...
#IBS matrix used fitered sites and run on phase 2 tank D2 males
######
#set working directory
setwd("/stor/work/Hofmann/projects/aburtoni_paternity_testing/Paternity_Testing/2bRAD_data/Data/Matrix/Phase2/TankD2")
...
samples=read.delim('bamsphase2d2males',header=F)
...
#subset IBS matrix by phase 2 Tank D2
...
g=graph_from_adjacency_matrix(m.ibs.D2.no.females,mode='undirected',weighted = TRUE)
layout <- layout.mds(g, dist = as.matrix(m.ibs.D2.no.females))
...
png("mds.network.phase.2.tank.D2.no.females.png", width = 1800, height = 700)
plot(g, layout = layout, vertex.size = 3, edge.label=round(E(g)$weight, 3), main='Tank D2 Males')
dev.off()

#just edges under 0.2
m.ibs.D2.no.females.2=ifelse(m.ibs.D2.no.females>=0.2,0,m.ibs.D2.no.females)
h=graph_from_adjacency_matrix(m.ibs.D2.no.females.2,mode='undirected',weighted = TRUE)
...
png("cut.mds.network.phase.2.tank.D2.no.females.png", width = 1800, height = 700)
plot(h, layout = layout, vertex.size = 3, edge.label=round(E(h)$weight, 3), main= 'Tank D2 Males Cut')
dev.off()

##ggnet2
color=V(h)$Sex 
color=gsub("3","male",color) 
color=gsub("1","brood",color)


ggnet2(h, mode= layout,label=T, edge.label=round(E(h)$weight, 3),color=color)+ggtitle('Tank D2 Males and Broods')

#subset IBS matrix by phase 1 males
m.ibs.D2.males=as.data.frame(m.ibs)
m.ibs.D2.males=m.ibs.D2.males[rownames(m.ibs.D2.males) %in% phase.2.D2.names.males,colnames(m.ibs.D2.males) %in% phase.2.D2.names.males]
m.ibs.D2.males=data.matrix(m.ibs.D2.males)

png("pvclut.phase.2.tank.D2.males.png", width = 1800, height = 700)
plot(pvclust(m.ibs.D2.males, method.hclust = "average",method.dist = 'euclidian'), main='Phase 2 Tank D2 Males')
dev.off()


#network undirected matrix with edge's labeled
g=graph_from_adjacency_matrix(m.ibs.D2.males,mode='undirected',weighted = TRUE)
layout <- layout.mds(g, dist = as.matrix(m.ibs.D2.males))

png("mds.network.phase.2.tank.D2.males.png", width = 1800, height = 700)
plot(g, layout = layout, vertex.size = 3, edge.label=round(E(g)$weight, 3), main='Tank D2 Males')
dev.off()

#just edges under 0.2
m.ibs.D2.males.2=ifelse(m.ibs.D2.males>=0.2,0,m.ibs.D2.males)
h=graph_from_adjacency_matrix(m.ibs.D2.males.2,mode='undirected',weighted = TRUE)

png("cut.mds.network.phase.2.tank.D2.males.png", width = 1800, height = 700)
plot(h, layout = layout, vertex.size = 3, edge.label=round(E(h)$weight, 3), main= 'Tank D2 Males Cut')
dev.off()

##ggnet2

ggnet2(h, mode= layout,label=T, edge.label=round(E(h)$weight, 3))+ggtitle('Cut Tank D2 Males')
ggsave('Cut Tank D2 Males.png', width = 1200, height = 600,units = 'mm')
...
m.ibs.D2.females=as.data.frame(m.ibs)
m.ibs.D2.females=m.ibs.D2.females[rownames(m.ibs.D2.females) %in% phase.2.D2.names.females,colnames(m.ibs.D2.females) %in% phase.2.D2.names.females]
m.ibs.D2.females=data.matrix(m.ibs.D2.females)

png("pvclut.phase.2.tank.D2.females.png", width = 1800, height = 700)
plot(pvclust(m.ibs.D2.females, method.hclust = "average",method.dist = 'euclidian'), main='Phase 2 Tank D2 females')
dev.off()
