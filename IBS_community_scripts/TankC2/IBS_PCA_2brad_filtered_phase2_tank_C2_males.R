...
#IBS matrix used fitered sites and run on phase 2 tank C2 males

#set working directory
setwd("/stor/work/Hofmann/projects/aburtoni_paternity_testing/Paternity_Testing/2bRAD_data/Data/Matrix/Phase2/TankC2")
...
samples=read.delim('bamsphase2c2males',header=F)
...
#subset IBS matrix by phase 2 Tank C2
...
g=graph_from_adjacency_matrix(m.ibs.C2.no.females,mode='undirected',weighted = TRUE)
layout <- layout.mds(g, dist = as.matrix(m.ibs.C2.no.females))
...
png("mds.network.phase.2.tank.C2.no.females.png", width = 1800, height = 700)
plot(g, layout = layout, vertex.size = 3, edge.label=round(E(g)$weight, 3), main='Tank C2 Males')
dev.off()

#just edges under 0.2
m.ibs.C2.no.females.2=ifelse(m.ibs.C2.no.females>=0.2,0,m.ibs.C2.no.females)
h=graph_from_adjacency_matrix(m.ibs.C2.no.females.2,mode='undirected',weighted = TRUE)
...
png("cut.mds.network.phase.2.tank.C2.no.females.png", width = 1800, height = 700)
plot(h, layout = layout, vertex.size = 3, edge.label=round(E(h)$weight, 3), main= 'Tank C2 Males Cut')
dev.off()

#subset IBS matrix by phase 1 males
m.ibs.C2.males=as.data.frame(m.ibs)
m.ibs.C2.males=m.ibs.C2.males[rownames(m.ibs.C2.males) %in% phase.2.C2.names.males,colnames(m.ibs.C2.males) %in% phase.2.C2.names.males]
m.ibs.C2.males=data.matrix(m.ibs.C2.males)

png("pvclut.phase.2.tank.C2.males.png", width = 1800, height = 700)
plot(pvclust(m.ibs.C2.males, method.hclust = "average",method.dist = 'euclidian'), main='Phase 2 Tank C2 Males')
dev.off()


#network undirected matrix with edge's labeled
g=graph_from_adjacency_matrix(m.ibs.C2.males,mode='undirected',weighted = TRUE)
layout <- layout.mds(g, dist = as.matrix(m.ibs.C2.males))

png("mds.network.phase.2.tank.C2.males.png", width = 1800, height = 700)
plot(g, layout = layout, vertex.size = 3, edge.label=round(E(g)$weight, 3), main='Tank C2 Males')
dev.off()

#just edges under 0.2
m.ibs.C2.males.2=ifelse(m.ibs.C2.males>=0.2,0,m.ibs.C2.males)
h=graph_from_adjacency_matrix(m.ibs.C2.males.2,mode='undirected',weighted = TRUE)

png("cut.mds.network.phase.2.tank.C2.males.png", width = 1800, height = 700)
plot(h, layout = layout, vertex.size = 3, edge.label=round(E(h)$weight, 3), main= 'Tank C2 Males Cut')
dev.off()

##ggnet2

ggnet2(h, mode= layout,label=T, edge.label=round(E(h)$weight, 3))+ggtitle('Cut Tank C2 Males')+scale_y_continuous(limits = c(-1,1), breaks = NULL)+scale_x_continuous(limits = c(-1,1), breaks = NULL)
...
m.ibs.C2.females=as.data.frame(m.ibs)
m.ibs.C2.females=m.ibs.C2.females[rownames(m.ibs.C2.females) %in% phase.2.C2.names.females,colnames(m.ibs.C2.females) %in% phase.2.C2.names.females]
m.ibs.C2.females=data.matrix(m.ibs.C2.females)

png("pvclut.phase.2.tank.C2.females.png", width = 1800, height = 700)
plot(pvclust(m.ibs.C2.females, method.hclust = "average",method.dist = 'euclidian'), main='Phase 2 Tank C2 females')
dev.off()
