...
#IBS matrix used fitered sites and run on phase 2 tank G2 males
######
#set working directory
setwd("/stor/work/Hofmann/projects/aburtoni_paternity_testing/Paternity_Testing/2bRAD_data/Data/Matrix/Phase2/TankG2")
...
samples=read.delim('bamsphase2g2males',header=F)
...
#subset IBS matrix by phase 2 Tank G2
...
g=graph_from_adjacency_matrix(m.ibs.G2.no.females,mode='undirected',weighted = TRUE)
layout <- layout.mds(g, dist = as.matrix(m.ibs.G2.no.females))
...
png("mds.network.phase.2.tank.G2.no.females.png", width = 1800, height = 700)
plot(g, layout = layout, vertex.size = 3, edge.label=round(E(g)$weight, 3), main='Tank G2 Males')
dev.off()

#just edges under 0.2
m.ibs.G2.no.females.2=ifelse(m.ibs.G2.no.females>=0.2,0,m.ibs.G2.no.females)
h=graph_from_adjacency_matrix(m.ibs.G2.no.females.2,mode='undirected',weighted = TRUE)
...
png("cut.mds.network.phase.2.tank.G2.no.females.png", width = 1800, height = 700)
plot(h, layout = layout, vertex.size = 3, edge.label=round(E(h)$weight, 3), main= 'Tank G2 Males Cut')
dev.off()

#subset IBS matrix by phase 1 males
m.ibs.G2.males=as.data.frame(m.ibs)
m.ibs.G2.males=m.ibs.G2.males[rownames(m.ibs.G2.males) %in% phase.2.G2.names.males,colnames(m.ibs.G2.males) %in% phase.2.G2.names.males]
m.ibs.G2.males=data.matrix(m.ibs.G2.males)

png("pvclut.phase.2.tank.G2.males.png", width = 1800, height = 700)
plot(pvclust(m.ibs.G2.males, method.hclust = "average",method.dist = 'euclidian'), main='Phase 2 Tank G2 Males')
dev.off()


#network undirected matrix with edge's labeled
g=graph_from_adjacency_matrix(m.ibs.G2.males,mode='undirected',weighted = TRUE)
layout <- layout.mds(g, dist = as.matrix(m.ibs.G2.males))

png("mds.network.phase.2.tank.G2.males.png", width = 1800, height = 700)
plot(g, layout = layout, vertex.size = 3, edge.label=round(E(g)$weight, 3), main='Tank G2 Males')
dev.off()

#just edges under 0.2
m.ibs.G2.males.2=ifelse(m.ibs.G2.males>=0.2,0,m.ibs.G2.males)
h=graph_from_adjacency_matrix(m.ibs.G2.males.2,mode='undirected',weighted = TRUE)

png("cut.mds.network.phase.2.tank.G2.males.png", width = 1800, height = 700)
plot(h, layout = layout, vertex.size = 3, edge.label=round(E(h)$weight, 3), main= 'Tank G2 Males Cut')
dev.off()
##########
...
m.ibs.G2.females=as.data.frame(m.ibs)
m.ibs.G2.females=m.ibs.G2.females[rownames(m.ibs.G2.females) %in% phase.2.G2.names.females,colnames(m.ibs.G2.females) %in% phase.2.G2.names.females]
m.ibs.G2.females=data.matrix(m.ibs.G2.females)

png("pvclut.phase.2.tank.G2.females.png", width = 1800, height = 700)
plot(pvclust(m.ibs.G2.females, method.hclust = "average",method.dist = 'euclidian'), main='Phase 2 Tank G2 females')
dev.off()

#########
## clustering  of tank G2 in phase 2 Remove BLUE.G2, GREEN.G2 
#remove BLUE.G2 and GREEN.G2
phase.2.G2.names.no.females.remove.bad=phase.2.G2.names.no.females[-c(2,3)]

#subset IBS matrix by phase 1 no females
m.ibs.G2.no.females.remove.bad=as.data.frame(m.ibs)
m.ibs.G2.no.females.remove.bad=m.ibs.G2.no.females.remove.bad[rownames(m.ibs.G2.no.females.remove.bad) %in% phase.2.G2.names.no.females.remove.bad,colnames(m.ibs.G2.no.females.remove.bad) %in% phase.2.G2.names.no.females.remove.bad]
m.ibs.G2.no.females.remove.bad=data.matrix(m.ibs.G2.no.females.remove.bad)

png("Remove.Green.Blue.males/pvclut.phase.2.tank.G2.no.females.remove.bad.png", width = 1800, height = 700)
plot(pvclust(m.ibs.G2.no.females.remove.bad, method.hclust = "average",method.dist = 'euclidian'), main='Phase 2 Tank G2 No Females Remove Bad')
dev.off()


#network undirected matrix with edge's labeled
g=graph_from_adjacency_matrix(m.ibs.G2.no.females.remove.bad,mode='undirected',weighted = TRUE)
layout <- layout.mds(g, dist = as.matrix(m.ibs.G2.no.females.remove.bad))
...
png("Remove.Green.Blue.males/mds.network.phase.2.tank.G2.no.females.remove.bad.png", width = 1800, height = 700)
plot(g, layout = layout, vertex.size = 3, edge.label=round(E(g)$weight, 3), main='Tank G2 Males Remove Bad')
dev.off()

#just edges under 0.2
m.ibs.G2.no.females.remove.bad.2=ifelse(m.ibs.G2.no.females.remove.bad>=0.2,0,m.ibs.G2.no.females.remove.bad)
h=graph_from_adjacency_matrix(m.ibs.G2.no.females.remove.bad.2,mode='undirected',weighted = TRUE)
...
png("Remove.Green.Blue.males/cut.mds.network.phase.2.tank.G2.no.females.remove.bad.png", width = 1800, height = 700)
plot(h, layout = layout, vertex.size = 3, edge.label=round(E(h)$weight, 3), main= 'Tank G2 Males Cut Remove Bad')
dev.off()

#subset IBS matrix by phase 1 males
phase.2.G2.names.males.remove.bad=phase.2.G2.names.males[-c(2,3)]

m.ibs.G2.males.remove.bad=as.data.frame(m.ibs)
m.ibs.G2.males.remove.bad=m.ibs.G2.males.remove.bad[rownames(m.ibs.G2.males.remove.bad) %in% phase.2.G2.names.males.remove.bad,colnames(m.ibs.G2.males.remove.bad) %in% phase.2.G2.names.males.remove.bad]
m.ibs.G2.males.remove.bad=data.matrix(m.ibs.G2.males.remove.bad)

png("Remove.Green.Blue.males/pvclut.phase.2.tank.G2.males.remove.bad.png", width = 1800, height = 700)
plot(pvclust(m.ibs.G2.males.remove.bad, method.hclust = "average",method.dist = 'euclidian'), main='Phase 2 Tank G2 Males Remove Bad')
dev.off()


#network undirected matrix with edge's labeled
g=graph_from_adjacency_matrix(m.ibs.G2.males.remove.bad,mode='undirected',weighted = TRUE)
layout <- layout.mds(g, dist = as.matrix(m.ibs.G2.males.remove.bad))

png("Remove.Green.Blue.males/mds.network.phase.2.tank.G2.males.remove.bad.png", width = 1800, height = 700)
plot(g, layout = layout, vertex.size = 3, edge.label=round(E(g)$weight, 3), main='Tank G2 Males Remove Bad')
dev.off()

#just edges under 0.2
m.ibs.G2.males.remove.bad.2=ifelse(m.ibs.G2.males.remove.bad>=0.2,0,m.ibs.G2.males.remove.bad)
h=graph_from_adjacency_matrix(m.ibs.G2.males.remove.bad.2,mode='undirected',weighted = TRUE)

png("Remove.Green.Blue.males/cut.mds.network.phase.2.tank.G2.males.remove.bad.png", width = 1800, height = 700)
plot(h, layout = layout, vertex.size = 3, edge.label=round(E(h)$weight, 3), main= 'Tank G2 Males Cut Remove Bad')
dev.off()
